...
export function renderReportes() {
  // El select de mes está siempre visible (lista y detalle) — se llena
  // siempre, aunque el resto del render se salte en modo lista.
  populateRepFilterMonth();

  // Vista de lista (sin reporte elegido) — no hay nada más visible que
  // renderizar; evita construir los 8 gráficos con canvases en display:none
  // (Chart.js los mediría con tamaño cero) cada vez que llega un cambio de
  // Firestore mientras el usuario solo está mirando la lista.
  if (document.getElementById("rep-detail-view")?.classList.contains("hidden")) return;
...
function showReportDetail(reportId) {
  document.getElementById("rep-list-view").classList.add("hidden");
  document.getElementById("rep-detail-view").classList.remove("hidden");
  document.querySelectorAll("#rep-detail-view [data-report]").forEach((el) => {
    el.classList.toggle("hidden", el.dataset.report !== reportId);
  });
  renderReportes();
}

export function showReportList() {
  document.getElementById("rep-detail-view").classList.add("hidden");
  document.getElementById("rep-list-view").classList.remove("hidden");
}

export function initReportes() {
  document.getElementById("rep-filter-month").addEventListener("change", renderReportes);
  document.querySelectorAll("#rep-list-view .rep-list-item").forEach((btn) => {
    btn.addEventListener("click", () => showReportDetail(btn.dataset.report));
  });
}
