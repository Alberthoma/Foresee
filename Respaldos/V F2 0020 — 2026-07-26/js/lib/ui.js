...
export function updateSelectionButtons(currentTab) {
  ...
  // En Registros, editar ya está disponible al tocar la fila (ver
  // toggleInfoRow) — el botón de edición en lote queda solo para
  // Recurrentes y Proyección, que no tienen ese atajo.
  btnEdit?.classList.toggle("hidden", count !== 1 || currentTab === "registros");
...
/* ===================================================================
   ...
   =================================================================== */
export function toggleInfoRow(btn, colspan, actions) {
  ...
  if (actions?.length) {
    ...
    actionsDiv.className = "info-row-actions";
    actions.forEach(({ action, id, icon, title, danger }) => {
      const actBtn = document.createElement("button");
      actBtn.className = danger ? "btn-icon btn-icon--danger" : "btn-icon";
      actBtn.dataset.action = action;
      actBtn.dataset.id = id;
      actBtn.title = title;
      actBtn.innerHTML = icon;
