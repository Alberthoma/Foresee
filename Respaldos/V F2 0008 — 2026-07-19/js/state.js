...
export function loadUserData(userId, onFirstPrefsLoad) {
  ...
  const goalsCol = collection(db, DB_COL(userId, "savingsGoals"));
  ...
  unsubscribers.push(
    ...
    onSnapshot(
      goalsCol,
      (snap) => {
        appState.savingsGoals = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
        scheduleRenderAll();
      },
      (err) => console.error("[Firestore] metas de ahorro:", err),
    ),
