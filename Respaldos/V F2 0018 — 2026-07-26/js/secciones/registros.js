...
export function initRegistros() {
  document.getElementById("btn-add-transaction").addEventListener("click", () => {
    openCalcModal({ onFinalize: saveTransaction, showTransferTab: true, onTransfer: openTransferModal });
  });
  ...
  document.getElementById("reg-tbody").addEventListener("click", (e) => {
    ...
    if (tr) {
      toggleInfoRow(tr, 8, [
        { action: "edit", id: tr.dataset.id, icon: ICON_PENCIL, title: "Editar" },
        { action: "delete", id: tr.dataset.id, icon: ICON_TRASH, title: "Eliminar", danger: true },
      ]);
    }
  });
