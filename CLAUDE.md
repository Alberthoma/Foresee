...
## 📡 Infraestructura de backend (Cloud Functions) — leer antes de tocar notificaciones

Hay **dos codebases de Cloud Functions distintas** para el mismo proyecto Firebase (`wittfinances-282f1`), y no se despliegan juntas. Esto se documenta acá específicamente porque el 2026-07-24 casi se rompe la que ya estaba en producción por no saber que existía — **leer esto antes de correr `firebase deploy --only functions` sin especificar nombres**.

### 1. `functions/` — notificaciones de la landing (activa, se despliega desde este repo)
`notifyNewLead` y `notifyNewSurvey` — avisan por correo (Gmail/Nodemailer) cuando alguien deja su correo o responde la encuesta en `landing/landing.html`. Ver `functions/README.md` para cómo desplegar (requiere `firebase deploy` manual, no pasa con el `git push` normal).

### 2. `functions-recordatorios-referencia/` — recordatorios de gastos recurrentes (activa, **NO se despliega desde este repo**)
`sendRecurringExpenseReminders` — función programada (corre sola todos los días 9:00 AM hora de Nueva York) que manda recordatorios de gastos recurrentes por **correo (SendGrid)** y/o **push al celular (Firebase Cloud Messaging)**, según los toggles de Configuración → Notificaciones de cada usuario. Existe y funciona en producción desde antes de esta sesión, pero **su código no vivía en ningún repo** — se descubrió su existencia el 2026-07-24 cuando `firebase deploy` avisó que la iba a borrar por no reconocerla en el código local (se abortó solo, no se perdió nada). Se trajo el código al repo como referencia documentada — ver `functions-recordatorios-referencia/README.md` para el detalle completo (qué hace, dependencias, cómo redesplegar si hace falta editarla).

⚠️ **Regla para no repetir el susto:** si en el futuro hay que desplegar funciones, usar siempre `firebase deploy --only functions:nombre1,nombre2` con los nombres explícitos — nunca `firebase deploy --only functions` a secas mientras las dos funciones no convivan en un mismo codebase desplegable.

### Sobre Twilio (aclarado el 2026-07-24)
El usuario recordaba haber configurado notificaciones vía Twilio (WhatsApp/SMS) "en la app anterior" y creía que se había trasladado a Foresee 2.0. Se investigó a fondo: **no hay ninguna mención de Twilio en ningún archivo de este repo, ni en `Proyecto-Anterior/`, ni en el código de ninguna de las dos Cloud Functions activas.** Sí existen credenciales de Twilio viejas (token, sid, número, `whatsapp_from`) guardadas en el sistema `runtimeconfig` legado de este proyecto Firebase, con fecha de octubre de 2025 (antes de que existiera Foresee 2.0) — no las usa ninguna función desplegada actualmente. Todo indica que es configuración huérfana de un intento que nunca se conectó a código, o de una función que existió y se borró sin dejar rastro. El mecanismo real y funcional de notificaciones al celular en Foresee 2.0 es **Firebase Cloud Messaging**, no Twilio (ver `js/firebase.js` → `loadMessaging()`, y `js/secciones/configuracion.js` → `handlePushReminderToggle()`).

### Diagnóstico del 2026-07-24: por qué no llegan las notificaciones push al usuario
Se verificó directo en Firestore (uid `Q3q4d1huSfYtImdq5RMsSHn0weW2`, el correcto para `albertomatosgil@gmail.com` — una nota anterior de esta misma sección tenía un UID desactualizado, corregido acá): el documento `user_data/preferences` **no tenía los campos `pushRemindersEnabled` ni `emailRemindersEnabled`** (nunca se habían activado los toggles en esta reconstrucción), y la colección `fcmTokens` estaba **vacía** (ningún dispositivo registrado). Como `sendRecurringExpenseReminders` solo manda recordatorios a quien tiene el toggle activo (`!!prefs.pushRemindersEnabled`), el sistema estaba funcionando correctamente — simplemente nunca se había activado para esta cuenta. **La función en sí está sana:** corre todos los días sin errores (`sent:X, failed:0, errors:0` en los logs de los últimos 7+ días).

**Resuelto el mismo día:** el usuario activó "Recordatorios Push" y "Recordatorios por correo" desde Configuración. Se verificó end-to-end forzando pruebas reales (no solo esperando el barrido de las 9am): un correo de prueba vía SendGrid llegó (HTTP 202), y un push directo a FCM llegó al iPhone registrado (HTTP 200, confirmado por el usuario). De paso se limpiaron 2 tokens FCM inválidos (uno de una prueba vieja en Chrome de escritorio, otro un duplicado muerto registrado en el mismo segundo que el token bueno del iPhone) — quedó solo el token válido del iPhone.

**Solución:** entrar a la app → Configuración → Notificaciones → activar "Recordatorios Push" (y "Recordatorios por correo" si también se quiere ese canal) desde el dispositivo donde se quieran recibir. Eso pide permiso del navegador, genera un token FCM, y lo guarda — a partir de ahí el barrido diario de las 9 AM debería empezar a mandar avisos cuando un gasto recurrente esté por vencer.

### Tarjetas y Préstamos suman recordatorios por correo/push (2026-07-24)
Antes, tarjetas y deudas solo tenían alerta **dentro de la app** (banner + notificación nativa del navegador, solo si estaba abierta, y solo por 80%+ de línea de crédito usada — nunca por fecha de vencimiento). Se extendió `sendRecurringExpenseReminders` (`functions-recordatorios-referencia/`) para que también recorra `creditCards` con el mismo esquema de 5/1/0 días que ya tenían los gastos recurrentes, usando los mismos toggles de Configuración → Notificaciones. Cambio aislado en su propio `try/catch` para no arriesgar el barrido de gastos recurrentes ya probado. `firebase.json` ahora declara dos codebases (`default` → `functions/`, `recordatorios` → `functions-recordatorios-referencia/`) — desplegar siempre con el nombre completo, ej. `firebase deploy --only functions:recordatorios:sendRecurringExpenseReminders`. Ver detalle en `functions-recordatorios-referencia/README.md`.

---

## Estado actual

- **Versión activa:** `V F2 0020` (2026-07-26) — Mismo patrón lista→detalle de Reportes (V F2 0019) aplicado a Configuración: 13 bloques apilados con scroll pasan a una lista de 12 ítems (ícono + título, sin descripción — Notificaciones agrupa sus 3 controles existentes, Copias de Seguridad agrupa Importar/Exportar + Exportar Datos), más 3 ítems nuevos al final (Privacidad/Términos/Cookies) que abren los modales ya existentes del footer. "Inicio" extendido para volver a la lista también en Configuración. Se eliminó `.cfg-grid` (layout de 3 columnas ya sin uso). Verificado con Playwright. Bump de `CACHE_NAME` (v20→v21).
- **Próxima versión:** `V F2 0021`
...
- **Último informe de actualización:** `Informes de actualización/V F2 0020 — 2026-07-26.md`
- **Último respaldo:** `Respaldos/V F2 0020 — 2026-07-26/`
- **Landing page (marketing):** `landing/landing.html` (movida por el usuario desde la raíz del repo a su propia carpeta `landing/`) — página de captación de clientes, **independiente del versionado `V F2 XXXX`** (no se carga desde `index.html`, no toca `css/`/`js/`/`sw.js`, no lleva footer de versión ni respaldo en `Respaldos/`). Sus links a la app usan `../index.html` (relativo, porque vive un nivel más adentro que antes). Ver `Informes de actualización/Landing Page — 2026-07-21.md` (tres rondas de cambios documentadas ahí). `landing/land.html` es la landing anterior que sirvió de referencia de estructura/activos; se conserva sin tocar. `landing/capturas-app/` se movió junto con `landing.html`.
  ...
  - **(2026-07-24)** Reglas de `firestore.rules` (colecciones `landing_leads`/`landing_survey`) publicadas por el usuario en Firebase Console — los formularios de la landing ya guardan datos reales.
  - **(2026-07-24) Notificaciones por correo (`functions/`) — desplegadas y verificadas:** Cloud Functions `notifyNewLead` y `notifyNewSurvey` (Node.js 20, 2nd Gen) activas en `wittfinances-282f1`, avisan a `albertomatosgil@gmail.com` cada vez que se crea un documento en `landing_leads`/`landing_survey` — antes había que revisar Firestore a mano. Envían por Gmail (Nodemailer) usando una contraseña de aplicación guardada como secreto (`GMAIL_USER`/`GMAIL_APP_PASSWORD`, Secret Manager). Verificado end-to-end con un documento de prueba (creado y borrado vía la API REST de Firestore) — la función respondió HTTP 200 sin errores. Coexiste con `sendRecurringExpenseReminders`, una Cloud Function preexistente (recordatorios de gastos recurrentes, usa SendGrid) que **no vive en este repo** — cuidado al desplegar con `firebase deploy --only functions` sin especificar nombres, porque el CLI intenta borrar cualquier función que no vea en `functions/` localmente; siempre usar `--only functions:notifyNewLead,notifyNewSurvey` para tocar solo las de acá. **No se despliega con el `git push` normal** — hace falta `firebase deploy --only functions:<nombre>` aparte si se edita `functions/index.js`; ver `functions/README.md` para el paso a paso completo. Requirió actualizar el Firebase CLI global (14→15) porque la versión vieja fallaba en silencio ("0 Functions Deployed" sin avisar) al desplegar funciones nuevas en un proyecto que ya tenía otra función existente.
  ...
  - **(2026-07-22, sexta ronda)** El hero y los dos bloques narrativos ("Del peso a la salida", "El verdadero tesoro") dejaron de mostrar sus ilustraciones como tarjetas con borde/sombra — ahora son fondos atmosféricos fundidos con `var(--bg)` (opacidad ~0.2 + degradado), igual al tratamiento de `landing/land.html` que el usuario pidió imitar. Se removieron los efectos genéricos (destellos ✦, resplandor flotante, brillo diagonal en botones, rotación de íconos de confianza). Orden de secciones revisado y dejado igual (ya seguía un arco narrativo razonable) — pendiente de confirmar con el usuario si su queja de "estructura" apuntaba a otra cosa.
  - **(2026-07-22, séptima ronda)** La opacidad/oscurecimiento de la sexta ronda dejaba las imágenes casi ilegibles — subida a ~0.42 (oscuro) / 0.3 (claro), filtro aclarado en vez de oscurecido, degradado más liviano.
  - **(2026-07-22, octava ronda)** Los 7 videos de "El corazón del flujo, en 7 pasos" (el mismo contenido que el tutorial dentro de la app) se sacaron de la landing — mucho compromiso para tráfico frío y redundante con el explorador y la guía. Se movieron a `landing/como-funciona.html`, embebidos dentro de las 7 herramientas correspondientes. Pendiente de decisión del usuario: aplicar la misma lógica al desplegable "Descubre cómo funciona" del explorador (¿recortarlo también de la landing?).
  - **(2026-07-22, novena ronda)** Se recortó el desplegable "Descubre cómo funciona" del explorador (misma lógica que los videos) — queda solo un link fijo "Ver la guía completa de esta herramienta →". La lista vertical de 12 botones se reemplazó por una **rueda giratoria**: los íconos orbitan sobre un anillo que gira solo (100s por vuelta), cada ícono se contra-rota para quedar siempre derecho, se pausa al pasar el mouse, y cae a una fila horizontal simple en mobile. Respeta `prefers-reduced-motion`.
  - **(2026-07-22, décima ronda)** Íconos de la rueda un 75% más grandes (60px→104px), con el radio de la órbita y el contenedor de la rueda reescalados en proporción (340px→580px de columna) para que no se superpongan.
...
- **Variables CSS solo para colores/tokens de tema** (`--color-*`) — nunca para espaciado o tipografía (padding, margin, gap, font-size): esos van en px literal (ej. `padding: 3px`, no `padding: var(--space-xs)`). Pedido explícito del usuario (2026-07-26) — le resulta engorroso saltar entre archivos CSS para saber a qué valor resuelve cada variable. Aplica solo a CSS nuevo; `--space-*`/`--font-*` ya existentes en `base.css`/`secciones.css` quedan igual hasta el refactor completo pendiente (ver "Deuda técnica").
...
| V F2 0014 | 2026-07-24 | Fix en Reportes: `currentRepMonth()` (único punto de verdad del mes en esa sección) validaba mal el valor del `<select>` de mes — cuando no había transacciones, el placeholder "Sin datos" (sin `value`) se colaba como si fuera un mes real, rompiendo los 6 gráficos con `RangeError: Invalid array length`. Le pasaba a cualquier usuario nuevo antes de su primera transacción, no solo en pruebas. Blindado con validación de formato "YYYY-MM" + fallback a `getCurrentMonthStr()`. Bump de `CACHE_NAME` (v14→v15). |
| V F2 0015 | 2026-07-24 | Fix del "reinicio" de la app al cambiar de pestaña del navegador: `onAuthStateChanged` (`js/main.js`) repetía el flujo completo de "primer login" (pantalla de carga, recarga total de datos, pantalla de bienvenida) cada vez que Firebase revalidaba una sesión ya activa al volver a la pestaña — no distinguía eso de un login nuevo. Se agregó `_loadedUid` para comparar contra el usuario ya cargado y solo repetir el flujo completo cuando cambia de verdad. De paso resuelve una fuga de listeners de Firestore duplicados en cada disparo redundante. Bump de `CACHE_NAME` (v15→v16). |
| V F2 0016 | 2026-07-26 | Rediseño de la barra de acciones para mobile: columna Saldo oculta en Registros (visible al tocar la fila, junto a editar/borrar, que se sumaron ahí); "Transferir" pasó de botón aparte a pestaña dentro de "+ Registrar" (`calc-modal.js` gana `onTransfer`/`showTransferTab`); "+ Registrar"/"+ Proyección" con íconos reales de Ingreso/Gasto/Transferencia y fondo neutro (igual que "Filtros"); mismo fondo neutro para los "+ Añadir" de Recurrentes/Tarjetas/Comunes/Metas, con textos más descriptivos; Registros y Proyección en una sola fila; Recurrentes/Comunes/Tarjetas/Metas con su botón centrado; "Eliminar selección" reducido a ícono 🗑️ sin fondo, al final de la fila; flecha de "Filtros" más grande sin empujar "Cómo funciona" a otra fila; menos espacio entre tab-bar/barra de acciones/tablas. De paso, se restauró el formato compacto de `index.html` tras detectar que Prettier lo había reformateado por completo. Bump de `CACHE_NAME` (v16→v17). |
| V F2 0017 | 2026-07-26 | El espacio entre el tab-bar y la barra de acciones seguía viéndose grande tras V F2 0016. Se probó y revirtió reducir `--tab-bar-mobile-h` (cortaba pestañas de 2 líneas). Causa real: `#main-wrapper` tenía su propio `padding-top` independiente del hueco reservado para el tab-bar fijo — se le agregó un `padding-top` menor puntual, sin tocar el tab-bar. Verificado con Playwright (instalado en scratchpad, no en el repo) comparando capturas a 390px. Bump de `CACHE_NAME` (v17→v18). |
| V F2 0018 | 2026-07-26 | Fix: al desplegar "Filtros" en Registros/Proyección los selects desordenaban toda la fila de íconos (el botón "Filtros" se hacía de ancho completo). Selects/checkbox separados en `<div class="filter-selects">` propio; `#reg-filter-group`/`#proj-filter-group` en `display: contents` para que "Filtros" sea un ícono más y los selects caigan en su propia fila (`flex-basis: 100%`, `order` alto). Registros/Proyección pasan a repartir sus 4 íconos en partes iguales (`even-secondary`, `justify-content: space-between`) en vez de agruparse a la derecha. Íconos de "+ Registrar"/"+ Proyección" de 22px a 25px. Verificado con Playwright a 390px. Bump de `CACHE_NAME` (v18→v19). |
| V F2 0019 | 2026-07-26 | Reportes rediseñado: lista (ícono/título/descripción) en vez de 8 gráficos apilados con scroll; al tocar un ítem se abre su detalle individual, y "Inicio" vuelve a la lista (no al dashboard) mientras se ve un reporte — sin botón "Volver" nuevo. En mobile se quitó "Filtros" de Reportes (select de mes siempre visible y centrado). Fix de paso: botón "Ayuda" ovalado en escritorio (`line-height:0` rompía su texto/emoji) y el select de mes que no se llenaba en la vista de lista. Verificado con Playwright, incluida una prueba con datos simulados vía `import("/js/state.js")`. Bump de `CACHE_NAME` (v19→v20). |
| V F2 0020 | 2026-07-26 | Mismo patrón lista→detalle de Reportes aplicado a Configuración: 13 bloques con scroll pasan a una lista de 12 ítems (Notificaciones agrupa sus 3 controles, Copias de Seguridad agrupa Importar/Exportar + Exportar Datos), más Privacidad/Términos/Cookies al final (abren los modales ya existentes del footer, sin duplicar contenido). "Inicio" extendido para volver a la lista también acá. `.cfg-grid` (layout de 3 columnas) eliminado por quedar sin uso. Verificado con Playwright. Bump de `CACHE_NAME` (v20→v21). |
...
- ~~**"Reinicio" de la app al cambiar de pestaña del navegador**~~ — **Resuelto el mismo día (2026-07-24), causa real identificada — no era la app.** No era Live Server (se confirmó que pasaba en el sitio publicado, en Chrome/Edge de escritorio, de forma instantánea). Se investigó y arregló un bug real en el camino (`onAuthStateChanged` en `js/main.js` repetía todo el flujo de "primer login" cada vez que Firebase revalidaba la sesión — ver `Informes de actualización/V F2 0015 — 2026-07-24.md`), pero **ese no era el origen del reinicio que reportaba el usuario**: la prueba decisiva fue la consola del navegador con "Preserve log" activado, que mostró literalmente `Navigated to https://alberthoma.github.io/Foresee/` — un recargo real de la página, no un problema del estado interno de la app. Probando en una ventana de incógnito (sin extensiones) el problema no ocurría, lo que aisló la causa a una extensión de Chrome. Confirmado por el usuario: **la extensión "IDM Integration Module" (de Internet Download Manager) era la responsable** — al desactivarla, el reinicio desapareció por completo. No hay nada para arreglar en el código de Foresee. **Para una futura sesión:** si un usuario reporta que la app "se reinicia" o "recarga sola" al cambiar de pestaña, probar primero en una ventana de incógnito antes de sospechar del código — si ahí no pasa, es una extensión del navegador del usuario, no un bug de la app.
...
- **Refactor de variables CSS de espaciado/fuente pendiente (2026-07-26)** — el usuario quiere convertir todo `--space-*`/`--font-*` existente en `css/base.css`/`css/secciones.css` a valores px literales (ver regla nueva en "Interfaz y estilos"), aceptando que es "mucho trabajo". No programado todavía — esperar a que el usuario lo pida explícitamente antes de emprenderlo; cuando se haga, ir regla por regla verificando visualmente con Playwright a 390px (no hacerlo como reemplazo masivo a ciegas).
