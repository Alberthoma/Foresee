...
- **Versión activa:** `V F2 0015` (2026-07-24) — fix del "reinicio" de la app al cambiar de pestaña: `onAuthStateChanged` (`js/main.js`) trataba cada disparo con usuario válido como un login nuevo (pantalla de carga + recarga completa de datos + pantalla de bienvenida otra vez), incluso cuando Firebase solo revalidaba una sesión ya activa al volver a la pestaña. Ahora se compara el `uid` contra el último cargado y se ignoran los disparos redundantes. Resuelve también una fuga de listeners de Firestore duplicados en cada disparo redundante.
...
- ~~**"Reinicio" de la app al cambiar de pestaña del navegador**~~ — **Resuelto en V F2 0015** (2026-07-24). No era Live Server (se confirmó que pasaba en el sitio publicado, en Chrome/Edge de escritorio, de forma instantánea): la causa real era que `onAuthStateChanged` (`js/main.js`) repetía todo el flujo de "primer login" (pantalla de carga, recarga completa de datos, pantalla de bienvenida) cada vez que Firebase revalidaba la sesión al volver a la pestaña. Ver `Informes de actualización/V F2 0015 — 2026-07-24.md`. Pendiente que el usuario confirme en producción que ya no vuelve a pasar.
