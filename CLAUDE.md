...
Se verificó directo en Firestore (uid `1xkibFZWAvc6yqTLI7v8HqUMD5j2`): el documento `user_data/preferences` **no tiene los campos `pushRemindersEnabled` ni `emailRemindersEnabled`** (nunca se activaron los toggles en esta reconstrucción, o se perdieron en algún reinicio), y la colección `fcmTokens` está **vacía** (ningún dispositivo registrado). Como `sendRecurringExpenseReminders` solo manda recordatorios a quien tiene el toggle activo (`!!prefs.pushRemindersEnabled`), el sistema está funcionando correctamente — simplemente nunca se activó para esta cuenta. **La función en sí está sana:** corre todos los días sin errores (`sent:X, failed:0, errors:0` en los logs de los últimos 7+ días).
